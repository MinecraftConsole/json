...
void LocalPlayer::aiStep()
{
	...
	{
		int iPad = GetXboxPad();
		bool keyboardGameplay = (iPad == ProfileManager.GetPrimaryPad()) && (minecraft->screen == NULL) && !ui.GetMenuDisplayed(iPad);
		if(keyboardGameplay)
		{
			keyboardSprintHeld = Keyboard::isKeyDown(Keyboard::KEY_LCONTROL) || Keyboard::isKeyDown(Keyboard::KEY_RCONTROL);
		}
	}
	...
	{
		if( keyboardSprintHeld && input->ya >= runTreshold )
		{
			setSprinting(true);
			sprintTriggerTime = 0;
			sprintTriggerRegisteredReturn = false;
