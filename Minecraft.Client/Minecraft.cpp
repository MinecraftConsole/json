...
#ifdef _WINDOWS64
	// Windows64 build: force a single fullscreen viewport.
	// Prevents UI/crosshair being rendered in split sections.
	for( int i = 0; i < XUSER_MAX_COUNT; i++ )
	{
		...
		//while (running)
		{
			...
			for( int i = 0; i < XUSER_MAX_COUNT; i++ )
			{
				...
				if ( m_pPsPlusUpsell != NULL && m_pPsPlusUpsell->hasResponse() && m_pPsPlusUpsell->m_userIndex == i )
				{
					...
					if(localplayers[i])
				{
					bool keyboardMousePrimary = (i == ProfileManager.GetPrimaryPad()) && (options != NULL) && (screen == NULL) && !ui.GetMenuDisplayed(i);
					bool keyboardJumpPressed = keyboardMousePrimary && Keyboard::isKeyPressed(options->keyJump->key);
					bool mouseUsePressed = keyboardMousePrimary && Mouse::isButtonPressed(1);
					bool keyboardInventoryPressed = keyboardMousePrimary && Keyboard::isKeyPressed(options->keyBuild->key);
					bool mouseActionPressed = keyboardMousePrimary && Mouse::isButtonPressed(0);
					bool keyboardPausePressed = keyboardMousePrimary && Keyboard::isKeyPressed(Keyboard::KEY_ESCAPE);
					bool keyboardDropPressed = keyboardMousePrimary && Keyboard::isKeyPressed(options->keyDrop->key);
					bool keyboardSneakPressed = keyboardMousePrimary && Keyboard::isKeyPressed(options->keySneak->key);
					bool keyboardSneakDown = keyboardMousePrimary && Keyboard::isKeyDown(options->keySneak->key);

					// 4J-PB - add these to check for coming out of idle
					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_JUMP) || keyboardJumpPressed)				localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_JUMP;
					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_USE) || mouseUsePressed)					localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_USE;

					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_INVENTORY) || keyboardInventoryPressed)				localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_INVENTORY;
					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_ACTION) || mouseActionPressed)					localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_ACTION;
					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_CRAFTING))				localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_CRAFTING;
					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_PAUSEMENU) || keyboardPausePressed)
					{
...
#endif
					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_DROP) || keyboardDropPressed)					localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_DROP;

					// 4J-PB - If we're flying, the sneak needs to be held on to go down
					if(localplayers[i]->abilities.flying)
					{
						if(InputManager.ButtonDown(i, MINECRAFT_ACTION_SNEAK_TOGGLE) || keyboardSneakDown)			localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_SNEAK_TOGGLE;
					}
					else
					{
						if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_SNEAK_TOGGLE) || keyboardSneakPressed)		localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_SNEAK_TOGGLE;
					}
...
void Minecraft::tick(bool bFirst, bool bUpdateTextures)
{
	...
	*/
	...
	{
		// Keep the event queues bounded while no GUI screen is consuming them.
		while (Keyboard::next()) {}
		while (Mouse::next()) {}
	}
	...
	if (screen == NULL && !ui.GetMenuDisplayed(iPad) )
	{
		...
		*piAction=-1;
		...
		{
			int mouseWheel = Mouse::getWheelDelta();
			if (mouseWheel > 0) wheel = 1;
			else if (mouseWheel < 0) wheel = -1;
		}
		...
		{
			bool mouseActionDown = (iPad == ProfileManager.GetPrimaryPad()) && Mouse::isButtonDown(0);
			bool actionDown = InputManager.ButtonDown(iPad, MINECRAFT_ACTION_ACTION) || mouseActionDown;
			...
			if (actionDown && ticks - player->lastClickTick[0] >= timer->ticksPerSecond / 4)
			{
		...
		*/
		bool mouseUseDown = (iPad == ProfileManager.GetPrimaryPad()) && Mouse::isButtonDown(1);
		bool useDown = InputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE) || mouseUseDown;
		if( player->isUsingItem() )
		{
			if(!useDown) gameMode->releaseUsingItem(player);
		}
		else if( gameMode->isInputAllowed(MINECRAFT_ACTION_USE) )
		{
			if( player->abilities.instabuild )
			{
				...
				bool didClick = player->creativeModeHandleMouseClick(1, useDown );
				...
				if( player->lastClickState == LocalPlayer::lastClick_oldRepeat )
				{
					...
					else
					{
						...
						if (useDown && ticks - player->lastClickTick[1] >= timer->ticksPerSecond / 4)
						{
		...
		if(iPad == ProfileManager.GetPrimaryPad())
		{
			...
			{
				if(Keyboard::isKeyDown(Keyboard::KEY_1 + hotbarSlot))
				{
					directHotbarSlot = hotbarSlot;
					break;
				}
			}
		}
		...
		{
			player->inventory->selected = directHotbarSlot;
			selected = true;
