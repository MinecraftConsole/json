...
HRESULT CScene_SettingsControl::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	...
	const int mouseSensitivityMax = 120;
	int mouseSensitivity = app.GetGameSettings(m_iPad,eGameSetting_Sensitivity_InMenu);
	if(mouseSensitivity > mouseSensitivityMax)
	{
		mouseSensitivity = mouseSensitivityMax;
		app.SetGameSettings(m_iPad,eGameSetting_Sensitivity_InMenu,mouseSensitivity);
	}
	m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INMENU].SetRange(0, mouseSensitivityMax);
	m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INMENU].SetValue(mouseSensitivity);
	swprintf( (WCHAR *)TempString, 256, L"Sensibilidad del Mouse: %d%%", mouseSensitivity);	
...
HRESULT CScene_SettingsControl::OnNotifyValueChanged( HXUIOBJ hObjSource, XUINotifyValueChanged* pNotifyValueChanged, BOOL& bHandled )
{
	...
	{
		int value = pNotifyValueChanged->nValue;
		if(value > 120) value = 120;
		app.SetGameSettings(m_iPad,eGameSetting_Sensitivity_InMenu,value);
		swprintf( (WCHAR *)TempString, 256, L"Sensibilidad del Mouse: %d%%", value);		
		m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INMENU].SetText(TempString);
	}
	...
	{		
		...
		for(int i=0;i<SLIDER_SETTINGS_CONTROL_MAX;i++)
		{
			...
			case SLIDER_SETTINGS_SENSITIVITY_INMENU: // 3 digits
				swprintf( (WCHAR *)TempString, 256, L"Sensibilidad del Mouse: %d%%", 120);	
				break;
			}
