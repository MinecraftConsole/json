...
void UIControl_PlayerSkinPreview::SetRotation(int xRot, int yRot)
{
	if(xRot > 22) xRot = 22;
	else if(xRot < -22) xRot = -22;
...
void UIControl_PlayerSkinPreview::SetStandingPose(bool standing)
{
	m_currentAnimation = standing ? e_SkinPreviewAnimation_Idle : e_SkinPreviewAnimation_Walking;
}
...
void UIControl_PlayerSkinPreview::renderScreenRect(float x, float y, float width, float height)
{
	Minecraft *pMinecraft=Minecraft::GetInstance();
	...
	glTranslatef(x + (width * 0.5f), y + height - 3.5f, 50.0f);

	float ss = width / (m_fScreenWidth / m_fScreenHeight);
...
void UIControl_PlayerSkinPreview::render(EntityRenderer *renderer, double x, double y, double z, float rot, float a)
{
	...
	float ws = 0.0f;
	float wp = 0.0f;
	if(m_currentAnimation == e_SkinPreviewAnimation_Walking && !m_bAnimatingToFacing)
	{
		m_walkAnimSpeedO = m_walkAnimSpeed;
		m_walkAnimSpeed += (0.1f - m_walkAnimSpeed) * 0.4f;
		m_walkAnimPos += m_walkAnimSpeed;
		ws = m_walkAnimSpeedO + (m_walkAnimSpeed - m_walkAnimSpeedO) * a;
		wp = m_walkAnimPos - m_walkAnimSpeed * (1 - a);
	}
	else
	{
		m_walkAnimSpeedO = 0.0f;
