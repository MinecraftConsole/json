...
{
	if(fTouchPos < 0.0f) fTouchPos = 0.0f;
	if(fTouchPos > 1.0f) fTouchPos = 1.0f;

	// Force slider movement to discrete steps (same behavior as left/right input).
	const int range = m_max - m_min;
	if(range > 0)
	{
		const float stepCount = (float)range;
		float quantized = (float)((int)(fTouchPos * stepCount + 0.5f)) / stepCount;
