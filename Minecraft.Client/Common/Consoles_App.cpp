...
#include "..\..\Minecraft.Client\UserData_Info.h"
...
#include "..\Windows64\NetworkPlayerXbox.h"
...
//#ifdef __PS3__
	...
	{
		GameSettingsA[iPad]->ucLanguage = savedLang;
	}
	else
	{
		GameSettingsA[iPad]->ucLanguage = MINECRAFT_LANGUAGE_SPANISH; // force Spanish on Windows64 if no saved preference
	}
	#else
...
int CMinecraftApp::OldProfileVersionCallback(LPVOID pParam,unsigned char *pucData, const unsigned short usVersion, const int iPad)
{
	...
	case PROFILE_VERSION_2:
		...
		{	
			...
			// PS3 1.05 - added Greek
			#if defined(_WINDOWS64)
			pGameSettings->ucLanguage = MINECRAFT_LANGUAGE_SPANISH; // force Spanish on Windows64
			#else
...
void CMinecraftApp::SetPlayerSkin(int iPad,DWORD dwSkinId)
{
	...
	if(iPad == ProfileManager.GetPrimaryPad())
	{
		UserData_Info::SetSkinId((unsigned int)dwSkinId);
	}
#endif
...
#endif
	{
		dwSkin = GameSettingsA[iPad]->dwSelectedSkin;
	}
	wchar_t chars[256];
